...
(function() {
  var HAUTEUR_CASE, LARGEUR_CASE, anim;
  ...
  this.Board = (function() {
    ...
    Board.prototype.init = function() {
      var joueur, _fn, _fn1, _i, _j, _len, _len1, _ref, _ref1,
        _this = this;
      _ref = ['A', 'B', 'C', 'D'];
      _fn = function(joueur) {
        var c, _j, _results;
        _results = [];
        for (c = _j = -4; _j <= 21; c = ++_j) {
          _results.push((function(c) {
            return _this.drawCase(joueur + c);
          })(c));
        }
        return _results;
      };
      ...
      this.drawCase('M');
      _ref1 = ['A', 'B', 'C', 'D'];
      _fn1 = function(joueur) {
        var c, _k, _results;
        _results = [];
        for (c = _k = -4; _k <= -1; c = ++_k) {
          _results.push((function(c) {
            var slot;
            slot = joueur + c;
            return new Pawn(_this, slot, joueur);
          })(c));
        }
        return _results;
      };
      ...
      this.majCourante();
      this.mains = {};
      this.mains['A'] = this.paper.text(70, 80, "");
      this.mains['B'] = this.paper.text(335, 90, "");
      this.mains['C'] = this.paper.text(330, 320, "");
      this.mains['D'] = this.paper.text(60, 310, "");
      return this.tas = this.paper.text(220, 170, "");
    };

    Board.prototype.xy = function(_pos) {
      return [_pos[0] * LARGEUR_CASE, _pos[1] * HAUTEUR_CASE];
    };

    Board.prototype.position = function(val) {
      var nval, res;
      if (val === 'M') {
        return [10, 10];
      }
      ...
      switch (val[0]) {
        case 'A':
          return res;
        case 'B':
          return [20 - res[1], res[0]];
        case 'C':
          return [20 - res[0], 20 - res[1]];
        case 'D':
          return [res[1], 20 - res[0]];
      }
    };

    Board.prototype.xy_position = function(val) {
      return this.xy(this.position(val));
    };

    Board.prototype.couleur = function(j) {
      switch (j) {
        case 'A':
          return "#ff0";
        case 'B':
    ...
    Board.prototype.drawCase = function(_pos) {
      var c2, circle, p;
      p = this.xy(this.position(_pos));
      circle = this.paper.ellipse(p[0], p[1], 10, 9).attr("fill", this.couleur(_pos[0]));
      if (_pos[1] === "-") {
        return c2 = circle.clone().attr("fill", "000").attr("opacity", "0.7");
      }
    };

    Board.prototype.prochainPion = function(_j) {
      var i, retour, _fn, _i,
        _this = this;
      retour = null;
      _fn = function(i) {
    ...
    Board.prototype.ecurieLibre = function(_j) {
      var i, retour, _fn, _i,
        _this = this;
      retour = null;
      _fn = function(i) {
    ...
    Board.prototype.execute = function(args) {
      var joueur;
      joueur = args[0];
      switch (args[1]) {
        case 'SORT':
          this.pions[this.prochainPion(joueur)].moveTo(joueur + "0", true);
          break;
        case 'DEPLACE':
          this.pions[args[2]].moveTo(args[3]);
          break;
        case 'MANGE':
          this.mange(args[2], args[3]);
          break;
        case 'ECHANGE':
          this.echange(args[2], args[3]);
          break;
        case 'PIOCHE':
          this.pioche(joueur, args[2]);
          break;
        case 'POSE':
          this.pose(joueur, args[2]);
          break;
        case 'JETTE':
          this.jette(joueur);
      }
      return $("#move").append("<p>" + args.join(" ") + "</p>");
    };

    Board.prototype.undo = function(args) {
      var joueur;
      joueur = args[0];
      switch (args[1]) {
        case 'SORT':
          this.pions[joueur + "0"].moveTo(this.ecurieLibre(joueur));
          break;
        case 'DEPLACE':
          this.pions[args[3]].moveTo(args[2]);
          break;
        case 'MANGE':
          this.pions[this.prochainPion(args[2])].moveTo(args[3]);
          break;
        case 'ECHANGE':
          this.echange(args[2], args[3]);
          break;
        case 'PIOCHE':
          this.depioche(joueur, args[2]);
          break;
        case 'POSE':
          this.pioche(joueur, args[2]);
      }
    ...
    Board.prototype.pioche = function(_j, _carte) {
      this.cartes[_j].push(_carte);
      return this.majCartes(_j);
    };

    Board.prototype.pose = function(_j, _carte) {
      this.tas.attr("text", _carte);
      this.cartes[_j].remove(_carte);
      return this.majCartes(_j);
    };

    Board.prototype.depioche = function(_j, _carte) {
      this.cartes[_j].remove(_carte);
      return this.majCartes(_j);
    };

    Board.prototype.majCartes = function(_j) {
      return this.mains[_j].attr("text", this.cartes[_j]);
    };

    Board.prototype.jette = function(_j) {
      this.cartes[_j] = [];
      return this.majCartes(_j);
    };

    Board.prototype.mange = function(_j, _pos) {
      var nouvellePos, pion, pionMangeur;
      pion = this.manges[_pos] ? this.manges[_pos] : this.pions[_pos];
      if (pion.joueur !== _j) {
        write("ATTENTION BUG " + _j + " isnt " + pion.joueur);
      }
      nouvellePos = this.ecurieLibre(_j);
      pionMangeur = this.pions[_pos];
      pion.moveTo(nouvellePos);
      this.pions[nouvellePos] = pion;
      return this.pions[_pos] = pionMangeur;
    };

    Board.prototype.echange = function(_pos1, _pos2) {
      var pion1, pion2;
      pion1 = this.pions[_pos1];
      pion2 = this.pions[_pos2];
      pion1.moveTo(this.ecurieLibre(pion1.joueur));
      pion2.moveTo(_pos1);
      return pion1.moveTo(_pos2);
    };

    Board.prototype.next = function() {
      var n, _fn, _i, _ref, _ref1,
        _this = this;
      if (this.lcourante === this.log.actions.length - 1) {
        return;
      }
      $("#move").html("");
      $("#move").append("<p><strong>" + "Coup n°" + (this.lcourante + 1) + "</strong></p>");
      _fn = function(n) {
        var args;
        args = _this.log.lines[n].split(" ");
        return _this.execute(args);
      };
      for (n = _i = _ref = this.log.actions[this.lcourante] + 1, _ref1 = this.log.actions[this.lcourante + 1]; _ref <= _ref1 ? _i <= _ref1 : _i >= _ref1; n = _ref <= _ref1 ? ++_i : --_i) {
        _fn(n);
      }
      this.lcourante++;
      return this.majCourante();
    };
    ...
    Board.prototype.prev = function() {
      ...
      $("#move").append("<p><strong>" + "Coup n°" + this.lcourante + " (prochain)</strong></p>");
      _fn = function(n) {
        var args;
        args = _this.log.lines[n].split(" ");
        return _this.undo(args);
      };
      for (n = _i = _ref = this.log.actions[this.lcourante - 1] + 1, _ref1 = this.log.actions[this.lcourante]; _ref <= _ref1 ? _i <= _ref1 : _i >= _ref1; n = _ref <= _ref1 ? ++_i : --_i) {
        _fn(n);
      }
      this.lcourante--;
      return this.majCourante();
    ...
    Board.prototype.majCourante = function() {
      return $('#courante').html(this.lcourante);
    };

    Board.prototype.gotoMove = function(_index) {
      var i, _i, _j, _ref, _ref1, _ref2, _results, _results1;
      if (_index > this.lcourante) {
        _results = [];
        for (i = _i = _ref = this.lcourante, _ref1 = _index - 1; _ref <= _ref1 ? _i <= _ref1 : _i >= _ref1; i = _ref <= _ref1 ? ++_i : --_i) {
        ...
        if (_index < this.lcourante) {
          _results1 = [];
          for (i = _j = _index, _ref2 = this.lcourante - 1; _index <= _ref2 ? _j <= _ref2 : _j >= _ref2; i = _index <= _ref2 ? ++_j : --_j) {
            _results1.push(this.prev());
          }
          return _results1;
        }
      }
...
(function() {
  this.Gamechart = (function() {
    function Gamechart(_log, _onClick) {
      Morris.Area({
        element: 'gamechart',
        data: _log.data,
        xkey: 'n',
...
(function() {
  this.Log = (function() {
    function Log(_log) {
      var actions, data, line, numAction, numLigne, vals, _fn, _i, _len, _ref,
        _this = this;
      numLigne = 0;
      numAction = 1;
      data = [];
...
(function() {
  this.Pawn = (function() {
    function Pawn(_board, _pos, _j, _bloqueur) {
      var c1, c2, c3, c4, c5, c6, coul, h, p, pDest, paper, rx, ry;
      if (_bloqueur == null) {
        _bloqueur = false;
      }
      this.anim = "<>";
      ...
      ry = 6;
      h = 7;
      coul = this.board.couleur(_j);
      this.joueur = _j;
      this.svgSet = this.board.paper.set();
      ...
      c3.attr("fill", coul);
      c4 = c3.clone();
      c4.attr("fill", _bloqueur ? "#000" : "#fff");
      c4.attr("opacity", "0.5");
      this.chapeau = c4;
    ...
    Pawn.prototype.moveTo = function(_pos, _bloqueur) {
      var p1, p2;
      if (_bloqueur == null) {
        _bloqueur = false;
      }
      this.board.pions[this.slot] = null;
      p1 = this.board.xy_position('M');
      p2 = this.board.xy_position(_pos);
      this.slot = _pos;
      this.svgSet.animate({
        transform: "t" + (p2[0] - p1[0]) + "," + (p2[1] - p1[1])
      }, 1000, this.anim);
      if (this.bloqueur) {
        this.bloqueur = false;
        this.chapeau.animate({
          fill: "#fff"
        }, 1000, this.anim);
      }
      if (_bloqueur) {
      ...
      if (this.board.pions[_pos] != null) {
        this.board.manges[_pos] = this.board.pions[_pos];
      }
      return this.board.pions[_pos] = this;
    };
